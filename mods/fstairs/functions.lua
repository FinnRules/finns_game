...
-- Register stair
-- Node will be called fstairs:stair_<subname>

function fstairs.register_stair(subname, recipeitem, groups, images, description,
	...
	new_groups.stair = 1
	warn_if_exists("fstairs:stair_" .. subname)
	minetest.register_node(":fstairs:stair_" .. subname, {
	...
	-- for replace ABM
	if replace then
		minetest.register_node(":fstairs:stair_" .. subname .. "upside_down", {
			replace_name = "fstairs:stair_" .. subname,
	...
	if recipeitem then
		...
		minetest.register_craft({
			output = "fstairs:stair_" .. subname .. " 8",
			recipe = {
				{"", "", recipeitem},
		...
		-- Use fstairs to craft full blocks again (1:1)
		minetest.register_craft({
			output = recipeitem .. " 3",
			recipe = {
				{"fstairs:stair_" .. subname, "fstairs:stair_" .. subname},
				{"fstairs:stair_" .. subname, "fstairs:stair_" .. subname},
			},
		})
...
-- Register slab
-- Node will be called fstairs:slab_<subname>

function fstairs.register_slab(subname, recipeitem, groups, images, description,
		sounds, worldaligntex)
	...
	local new_groups = table.copy(groups)
	new_groups.slab = 1
	warn_if_exists("fstairs:slab_" .. subname)
	minetest.register_node(":fstairs:slab_" .. subname, {
		description = description,
		drawtype = "nodebox",
		tiles = slab_images,
		paramtype = "light",
		...
		on_place = function(itemstack, placer, pointed_thing)
			...
			if under and under.name:find("^fstairs:slab_") then
	...
	-- for replace ABM
	if replace then
		minetest.register_node(":fstairs:slab_" .. subname .. "upside_down", {
			replace_name = "fstairs:slab_".. subname,
			groups = {slabs_replace = 1},
		})
	end
	...
	if recipeitem then
		minetest.register_craft({
			output = "fstairs:slab_" .. subname .. " 6",
			recipe = {
				{recipeitem, recipeitem, recipeitem},
			},
		})
...
-- Node will be called fstairs:stair_inner_<subname>

function fstairs.register_stair_inner(subname, recipeitem, groups, images,
	...
	warn_if_exists("fstairs:stair_inner_" .. subname)
	minetest.register_node(":fstairs:stair_inner_" .. subname, {
		description = description,
		drawtype = "nodebox",
		tiles = stair_images,
		paramtype = "light",
	...
	if recipeitem then
		minetest.register_craft({
			output = "fstairs:stair_inner_" .. subname .. " 7",
			recipe = {
				{"", recipeitem, ""},
				{recipeitem, "", recipeitem},
				{recipeitem, recipeitem, recipeitem},
		...
		if baseburntime > 0 then
			minetest.register_craft({
				type = "fuel",
				recipe = "fstairs:stair_inner_" .. subname,
...
-- Node will be called fstairs:stair_outer_<subname>

function fstairs.register_stair_outer(subname, recipeitem, groups, images,
		description, sounds, worldaligntex, full_description)
	-- Set backface culling and world-aligned textures
	local stair_images = {}
	for i, image in ipairs(images) do
	...
	warn_if_exists("fstairs:stair_outer_" .. subname)
	minetest.register_node(":fstairs:stair_outer_" .. subname, {
		description = description,
		drawtype = "nodebox",
		tiles = stair_images,
		paramtype = "light",
	...
	if recipeitem then
		minetest.register_craft({
			output = "fstairs:stair_outer_" .. subname .. " 6",
			recipe = {
				{"", recipeitem, ""},
				{recipeitem, recipeitem, recipeitem},
			},
		...
		if baseburntime > 0 then
			minetest.register_craft({
				type = "fuel",
				recipe = "fstairs:stair_outer_" .. subname,
...
-- Nodes will be called fstairs:{stair,slab}_<subname>

function fstairs.register_stair_and_slab(subname, recipeitem, groups, images,
		desc_stair, desc_slab, sounds, worldaligntex)
	fstairs.register_stair(subname, recipeitem, groups, images, desc_stair,
		sounds, worldaligntex)
	fstairs.register_stair_inner(subname, recipeitem, groups, images, desc_stair,
		sounds, worldaligntex)
	fstairs.register_stair_outer(subname, recipeitem, groups, images, desc_stair,
		sounds, worldaligntex)
	fstairs.register_slab(subname, recipeitem, groups, images, desc_slab,
		sounds, worldaligntex)
end

--[[
-- Local function so we can apply translations
function fstairs.my_register_stair_and_slab(subname, recipeitem, groups, images,
		desc_stair, desc_slab, sounds, worldaligntex)
	fstairs.register_stair(subname, recipeitem, groups, images, desc_stair,
		sounds, worldaligntex)
	fstairs.register_stair_inner(subname, recipeitem, groups, images, "",
		sounds, worldaligntex, "Inner " .. desc_stair)
	fstairs.register_stair_outer(subname, recipeitem, groups, images, "",
...
-- Register default fstairs and slabs
--[[
