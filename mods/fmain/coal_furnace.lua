...
function fmain.register_coal_craft(def)
	...
	table.insert(coal_furnace, def)
end

function fmain.register_coal_fuel(def)
	if not def.recipe then
		def.recipe = nil
	end
	...
	table.insert(coal_fuel, def)
end
	

function fmain.get_coal_furnacable(item)
	for i in pairs(coal_furnace) do
		if coal_furnace[i].recipe == item then
			return coal_furnace[i]
		end
	end
...
function fmain.get_coal_fuel(item)
	for i in pairs(coal_fuel) do
		if coal_fuel[i].recipe == item then
			return coal_fuel[i]
		end
	end
...
function fmain.get_coal_furnace_inactive_formspec()
	return "size[8,8.5]"..
		"list[context;src;2.75,0.5;1,1;]"..
		"list[context;fuel;2.75,2.5;1,1;]"..
...
local function allow_metadata_inventory_put(pos, listname, index, stack, player)
	...
	if listname == "fuel" then
		if fmain.get_coal_fuel(stack:get_name()).burntime ~= 0 then --probably broken
...
local function furnace_node_timer(pos, elapsed)
	...
	while elapsed > 0 and update do
		...
		local aftercooked
		cooked = fmain.get_coal_furnacable(itemInside) --Change me to a custom function
		aftercooked = fmain.get_coal_furnacable(itemInside)
		...
		if fuel_time < fuel_totaltime then
			...
			if cookable then
				...
				local afterfuel
				fuel = fmain.get_coal_fuel(fuelInside)
				afterfuel = fmain.get_coal_fuel(fuelInside)
...
--[[
	...
	if fuel_totaltime ~= 0 then
		...
		formspec = fmain.get_coal_furnace_active_formspec(fuel_percent, item_percent)
		swap_node(pos, "fmain:coal_furnace_active")
		...
		formspec = fmain.get_coal_furnace_inactive_formspec()
		swap_node(pos, "fmain:coal_furnace")
	...
	local infotext
	if active then
		infotext = "Coal Furnace active"
	else
		infotext = "Coal Furnace inactive"
	end
...
--	sounds = default.node_sound_stone_defaults(),
	...
	on_blast = function(pos)
		...
		drops[#drops+1] = "fmain:coal_furnace"
