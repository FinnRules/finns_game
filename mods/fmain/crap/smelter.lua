...
function fmain.get_hotbar_bg(x,y)
	local out = "" --declares out
	for i=0,7,1 do 
		out = out .."image["..x+i..","..y..";1,1;gui_hb_bg.png]" --creates an 8*1 hotbar formspec
	end
	return out
end
...
local function can_dig(pos, player) --Declares can_dig 
	local meta = minetest.get_meta(pos); --returns NodeMetaRef for node at pos (nodemetaref is "extra data and functionality stored in a node")
	local inv = meta:get_inventory() --gets an InvRef
...
local function furnace_node_timer(pos, elapsed)
	...
	if fuel_totaltime ~= 0 then
		...
		swap_node(pos, "fmain:furnace_active")
...
minetest.register_node("fmain:furnace", {
	description = "Furnace",
	tiles = {
...
minetest.register_node("fmain:furnace_active", {
	description = "Furnace",
	tiles = {
