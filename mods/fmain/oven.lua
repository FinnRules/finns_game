...
function fmain.register_oven_craft(def)
	...
	table.insert(oven, def)
end

function fmain.register_oven_fuel(def)
	if not def.recipe then
		def.recipe = nil
	end
	...
	table.insert(oven_fuel, def)
end
	

function fmain.get_ovenable(item)
	for i in pairs(oven) do
		if oven[i].recipe == item then
			return oven[i]
		end
	end
...
function fmain.get_oven_fuel(item)
	for i in pairs(oven_fuel) do
		if oven_fuel[i].recipe == item then
			return oven_fuel[i]
		end
	end
...
fmain.register_oven_craft({
	recipe = "fmining:iron_ore",
	cooktime = 150,
	output = "fmining:iron_ingot",
})

fmain.register_oven_fuel({
	recipe = "fmining:coal",
	burntime = 30,
})


function fmain.get_oven_active_formspec(fuel_percent, item_percent)
	return "size[8,8.5]"..
...
function fmain.get_oven_inactive_formspec()
	return "size[8,8.5]"..
		"list[context;src;2.75,0.5;1,1;]"..
		"list[context;fuel;2.75,2.5;1,1;]"..
...
local function allow_metadata_inventory_put(pos, listname, index, stack, player)
	...
	if listname == "fuel" then
		if fmain.get_oven_fuel(stack:get_name()).burntime ~= 0 then --probably broken
...
local function furnace_node_timer(pos, elapsed)
	...
	while elapsed > 0 and update do
		...
		local aftercooked
		cooked = fmain.get_ovenable(itemInside) --Change me to a custom function
		aftercooked = fmain.get_ovenable(itemInside)
		...
		if fuel_time < fuel_totaltime then
			...
			if cookable then
				...
				local afterfuel
				fuel = fmain.get_oven_fuel(fuelInside)
				afterfuel = fmain.get_oven_fuel(fuelInside)
...
--[[
	...
	if fuel_totaltime ~= 0 then
		...
		formspec = fmain.get_oven_active_formspec(fuel_percent, item_percent)
		swap_node(pos, "fmain:oven_active")
		...
		formspec = fmain.get_oven_inactive_formspec()
		swap_node(pos, "fmain:oven")
	...
	local infotext
	if active then
		infotext = "Oven active"
	else
		infotext = "Oven inactive"
	end
...
minetest.register_node("fmain:oven", {
	description = "oven",
	tiles = {
		"fmain_oven_top.png", "fmain_oven_bottom.png",
		"fmain_oven_side.png", "fmain_oven_side.png",
		"fmain_oven_side.png", "fmain_oven_front.png"
	},
	paramtype2 = "facedir",
...
minetest.register_node("fmain:oven_active", {
	description = "Oven",
	tiles = {
		"fmain_oven_top.png", 
		...
		"fmain_oven_side.png",
		{
			image = "fmain_oven_front_active.png",
