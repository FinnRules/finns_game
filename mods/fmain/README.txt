fmain in Finn's Game

Code by Emetchi LGPLv3, code by milcerine LGPLv3, portions used from Minetest Game

Textures:
Emetchi (CC-BY-SA 3.0)
	fmain_fluxed_coal_powder.png
	fmain_maple_fence.png
	fmain_maple_planks.png
	fmain_maple_sapling.png
	fmain_maple_sapling_green.png
	fmain_obsidian.png
	fmain_pine_wood_bark.png
	fmain_pine_wood_core.png
	fmain_pine_needles.png
	fmain_redwood_bark.png
	fmain_redwood_core.png
	fmain_redwood_fence.png
	fmain_redwood_leaves.png
	fmain_stick.png
	fmain_stone_axe.png
	fmain_white_oak_bark.png
	fmain_white_oak_core.png
	fmain_white_oak_planks.png
	fmain_wood_axe.png

Emetchi from Milcerine (CC-BY-SA 3.0)
	fmain_black_sandstone.png
	fmain_cherry_white_blossoms.png
	fmain_dirt_cherry.png
	fmain_dirt_cherry_white.png

Emetchi from SonorousChortle (CC-BY-SA 3.0)
	fmain_maple_bark.png
	fmain_maple_core.png
	fmain_maple_leaves_green.png

Emetchi from celeron55 (CC-BY-SA 3.0)
	fmain_coal_furnace_fire_bg.png
	fmain_coal_furnace_fire_fg.png

Emetchi from RobinxFox (CC-BY-SA 3.0)
	fmain_stone_brick.png

Milcerine (CC-BY-SA 3.0)
	fmain_baked_clay.png
	fmain_black_sand.png
	fmain_bronze_shovel.png
	fmain_bronze_sword.png
	fmain_cherry_bark.png
	fmain_cherry_blossoms.png
	fmain_cherry_core.png
	fmain_cherry_fence.png
	fmain_cherry_planks.png
	fmain_cherry_sapling.png
	fmain_cherry_white_sapling.png
	fmain_clay.png
	fmain_clay_lump.png
	fmain_diamond_shovel.png
	fmain_diamond_sword.png
	fmain_flint.png
	fmain_flower_bellflower.png
	fmain_flower_bells_of_ireland.png
	fmain_flower_dahlia.png
	fmain_flower_hellebore.png
	fmain_flower_nemophila.png
	fmain_flower_purple_orchid.png
	fmain_flower_red_hot_poker.png
	fmain_flower_rose.png
	fmain_flower_snowdrop.png
	fmain_grass.png
	fmain_grass_side.png
	fmain_gravel.png
	fmain_iron_shovel.png
	fmain_iron_sword.png
	fmain_lava_flowing_animated.png
	fmain_lava_source_animated.png
	fmain_moss.png
	fmain_moss_side.png
	fmain_poplar_sapling.png
	fmain_redwood_litter.png
	fmain_redwood_short_grass.png
	fmain_short_grass.png
	fmain_snow.png
	fmain_snow_side.png
	fmain_stone_shovel.png
	fmain_stone_sword.png
	fmain_tungsten_shovel.png
	fmain_tungsten_sword.png
	fmain_weeping_willow_bark.png
	fmain_weeping_willow_core.png
	fmain_weeping_willow_fence.png
	fmain_weeping_willow_leaves.png
	fmain_weeping_willow_planks.png
	fmain_weeping_willow_sapling.png
	fmain_white_sand.png
	fmain_wood_shovel.png
	fmain_wood_sword.png
	fmain_wool.png

Milcerine from Emetchi (CC-BY-SA 3.0)
	fmain_redwood_planks.png
	fmain_white_oak_fence.png
	fmain_white_oak_sapling.png

Milcerine from RobinFox (CC-BY-SA 3.0)
	fmain_poplar_fence.png

LizardWizard (CC-BY-SA 3.0)
	fmain_bronze_axe.png
	fmain_chest_front.png
	fmain_chest_inside.png
	fmain_chest_side.png
	fmain_chest_top.png
	fmain_diamond_axe.png
	fmain_iron_axe.png
	fmain_tungsten_axe.png

SonorousChortle (CC-BY-SA 3.0)
	fmain_dirt.png
	fmain_leaf_litter.png
	fmain_leaf_litter_side.png
	fmain_maple_leaves.png

RobinFox (CC-BY-SA 3.0)
	fmain_coal_furnace_bottom.png
	fmain_coal_furnace_front.png
	fmain_coal_furnace_front_active.png
	fmain_coal_furnace_side.png
	fmain_coal_furnace_top.png
	fmain_forge_bottom.png
	fmain_forge_front.png
	fmain_forge_front_active.png
	fmain_forge_side.png
	fmain_forge_top.png
	fmain_geothermal_furnace_bottom.png
	fmain_geothermal_furnace_front.png
	fmain_geothermal_furnace_front_active.png
	fmain_geothermal_furnace_side.png
	fmain_geothermal_furnace_top.png
	fmain_limestone.png
	fmain_limestone_chunk.png
	fmain_marble.png
	fmain_marble_brick.png
	fmain_nuclear_reactor_bottom.png
	fmain_nuclear_reactor_front.png
	fmain_nuclear_reactor_front_active.png
	fmain_nuclear_reactor_side.png
	fmain_nuclear_reactor_top.png
	fmain_oven_bottom.png
	fmain_oven_front.png
	fmain_oven_front_active.png
	fmain_oven_side.png
	fmain_oven_top.png
	fmain_polished_marble.png
	fmain_poplar_bark.png
	fmain_poplar_core.png
	fmain_poplar_leaves.png
	fmain_poplar_planks.png
	fmain_sand.png
	fmain_slate.png
	fmain_stone.png
	fmain_wood_stove_front.png
	fmain_wood_stove_front_active.png
	fmain_wood_stove_side.png
	fmain_wood_stove_topbottom.png

From Minetest Game:

Neuromancer from Brane praefect (CC-BY-SA 3.0):
	fmain_cobble.png

celeron55 (CC-BY-SA 3.0)
	fmain_fence_overlay.png
	fmain_fence_rail_overlay.png
	fmain_furnace_fire_bg.png
	fmain_furnace_fire_fg.png
	fmain_ladder.png
	fmain_sandstone.png

BlockMen (CC-BY-SA 3.0):
	gui_formbg.png
	gui_furnace_arrow_bg.png
	gui_furnace_arrow_fg.png
	gui_hotbar_bg.png

VanessaE (CC BY-SA 3.0):
	fmain_torch_animated.png
	fmain_torch_on_ceiling_animated.png
	fmain_torch_on_floor_animated.png
	fmain_torch_on_floor.png

Cisoun (CC-BY-SA 3.0):
	fmain_water.png

RealBadAngel (CC-BY-SA 3.0):
	fmain_water_source_animated.png
	fmain_water_flowing_animated.png

paramat (CC-BY-SA 3.0):
	gui_hotbar.png

Krock (CC0 1.0)
	fmain_glass.png
	fmain_glass_detail.png

++++++++++++
Schematics:
++++++++++++

Emetchi (CC-BY-SA 3.0)
	cherry_tree_white.mts
	maple_tree.mts
	redwood_tree.mts
	white_oak_tree.mts

milcerine (CC-BY-SA 3.0)
	cherry_tree.mts
	redwood_tree_alt1.mts
	redwood_tree_alt2.mts
	weeping_willow_tree.mts


