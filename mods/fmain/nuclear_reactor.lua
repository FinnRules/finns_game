...
function fmain.register_nuclear_craft(def)
	...
	table.insert(nuclear_reactor, def)
end

function fmain.register_nuclear_fuel(def)
	if not def.recipe then
		def.recipe = nil
	end
	...
	table.insert(nuclear_fuel, def)
end
	

function fmain.get_nukable(item)
	for i in pairs(nuclear_reactor) do
		if nuclear_reactor[i].recipe == item then
			return nuclear_reactor[i]
		end
	end
...
function fmain.get_nuclear_fuel(item)
	for i in pairs(nuclear_fuel) do
		if nuclear_fuel[i].recipe == item then
			return nuclear_fuel[i]
		end
	end
...
function fmain.get_nuclear_reactor_inactive_formspec()
	return "size[8,8.5]"..
		"list[context;src;2.75,0.5;1,1;]"..
		"list[context;fuel;2.75,2.5;1,1;]"..
...
local function allow_metadata_inventory_put(pos, listname, index, stack, player)
	...
	if listname == "fuel" then
		if fmain.get_nuclear_fuel(stack:get_name()).burntime ~= 0 then --probably broken
...
local function furnace_node_timer(pos, elapsed)
	...
	while elapsed > 0 and update do
		...
		local aftercooked
		cooked = fmain.get_nukable(itemInside) --Change me to a custom function
		aftercooked = fmain.get_nukable(itemInside)
		...
		if fuel_time < fuel_totaltime then
			...
			if cookable then
				...
				local afterfuel
				fuel = fmain.get_nuclear_fuel(fuelInside)
				afterfuel = fmain.get_nuclear_fuel(fuelInside)
...
--[[
	...
	if fuel_totaltime ~= 0 then
		...
		formspec = fmain.get_nuclear_reactor_active_formspec(fuel_percent, item_percent)
		swap_node(pos, "fmain:nuclear_reactor_active")
		...
		formspec = fmain.get_nuclear_reactor_inactive_formspec()
		swap_node(pos, "fmain:nuclear_reactor")
	...
	local infotext
	if active then
		infotext = "Nuclear Reactor  active"
	else
		infotext = "Nuclear Reactor inactive"
	end
...
minetest.register_node("fmain:nuclear_reactor", {
	description = "Nuclear Reactor",
	tiles = {
		"fmain_nuclear_reactor_top.png", "fmain_nuclear_reactor_bottom.png",
		"fmain_nuclear_reactor_side.png", "fmain_nuclear_reactor_side.png",
		"fmain_nuclear_reactor_side.png", "fmain_nuclear_reactor_front.png"
	},
	paramtype2 = "facedir",
...
--	sounds = default.node_sound_stone_defaults(),
	...
	on_blast = function(pos)
		...
		drops[#drops+1] = "fmain:nuclear_reactor"
...
minetest.register_node("fmain:nuclear_reactor_active", {
	description = "Nuclear Reactor",
	tiles = {
		...
		"fmain_nuclear_reactor_side.png",
		{
			image = "fmain_nuclear_reactor_front_active.png",
