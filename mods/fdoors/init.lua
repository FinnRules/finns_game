...
fdoors.registered_doors = {}
fdoors.registered_trapdoors = {}
fdoors.path = minetest.get_modpath("fdoors")
...
function fdoors.get(pos)
	local node_name = minetest.get_node(pos).name
	if fdoors.registered_doors[node_name] then
		...
		return {
			...
			open = function(self, player)
				...
				return fdoors.door_toggle(self.pos, nil, player)
			end,
	...
	elseif fdoors.registered_trapdoors[node_name] then
		-- A trapdoor
		return {
			pos = pos,
			open = function(self, player)
				...
				return fdoors.trapdoor_toggle(self.pos, nil, player)
			end,
			close = function(self, player)
				if not self:state() then
					return false
...
minetest.register_node("fdoors:hidden", {
...
function fdoors.door_toggle(pos, node, clicker)
	local meta = minetest.get_meta(pos)
...
function fdoors.register(name, def)
	...
	minetest.register_lbm({
		name = ":fdoors:replace_" .. name:gsub(":", "_"),
	...
	minetest.register_craftitem(":" .. name, {
		...
		on_place = function(itemstack, placer, pointed_thing)
			...
			if minetest.get_item_group(minetest.get_node(aside).name, "door") == 1 then
				...
				minetest.set_node(above, {name = "fdoors:hidden", param2 = (dir + 3) % 4})
			else
				minetest.set_node(pos, {name = name .. "_a", param2 = dir})
				minetest.set_node(above, {name = "fdoors:hidden", param2 = dir})
			end
	...
	if not def.sound_open then
		def.sound_open = "fdoors_door_open"
	end

	if not def.sound_close then
		def.sound_close = "fdoors_door_close"
	end
	...
	if not def.on_rightclick then
		def.on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
			fdoors.door_toggle(pos, node, clicker)
			return itemstack
		end
	end
	...
	fdoors.registered_doors[name .. "_a"] = true
	fdoors.registered_doors[name .. "_b"] = true
	fdoors.registered_doors[name .. "_c"] = true
	fdoors.registered_doors[name .. "_d"] = true
end
--[[
fdoors.register("door_wood", {
...
function fdoors.trapdoor_toggle(pos, node, clicker)
	node = node or minetest.get_node(pos)
...
function fdoors.register_trapdoor(name, def)
	if not name:find(":") then
		name = "fdoors:" .. name
	end
	...
	def.on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
		fdoors.trapdoor_toggle(pos, node, clicker)
		return itemstack
	end
	...
	fdoors.registered_trapdoors[name_opened] = true
	fdoors.registered_trapdoors[name_closed] = true
end
--[[
...
function fdoors.register_fencegate(name, def)
	local fence = {
		description = def.description,
		drawtype = "mesh",
		tiles = {},
	...
	fence_closed.mesh = "fdoors_fencegate_closed.obj"
	fence_closed.gate = name .. "_open"
	fence_closed.sound = "fdoors_fencegate_open"
	...
	fence_open.mesh = "fdoors_fencegate_open.obj"
	fence_open.gate = name .. "_closed"
	fence_open.sound = "fdoors_fencegate_close"
...
dofile(fdoors.path .. "/definitions.lua")
